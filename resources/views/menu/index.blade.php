...
@section('contents')
    <main class="container mx-auto mt-4 py-8 p-12 my-8">
        @if (auth()->user()->level == 'Admin')
            <!-- Menampilkan tombol untuk menambah menu dan melihat kategori jika pengguna adalah admin -->
            <div class="relative bottom-0 left-0 text-white p-4 fixed bottom-0 left-0 w-full">
                <div class="container mx-auto text-center">
                    <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center mr-3 md:mr-0" onclick="location.href='{{ route('menu.tambah') }}'" >Tambah Menu</button>
                    <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center mr-3 md:mr-0" onclick="location.href='{{ route('kategori') }}'" >Kategori</button>
                </div>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 card-container">
            @foreach ($data as $menu)
                <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow custom-card card">
                    <a href="#">
                        <!-- Menampilkan gambar dari item menu -->
                        <img src="{{ asset('storage/images/' . $menu->image) }}" alt="Makanan 1" class="w-full h-auto rounded-lg mb-4">
                    </a>
                    <div class="px-5 pb-5">
                        <a href="#">
                            <!-- Menampilkan nama dari item menu -->
              ...
              @if (auth()->user()->level == 'Admin')
                <div class="flex items-center mt-2.5 mb-5">
                  ...
                  <a href="{{ route('menu.hapus', $menu->id) }}"><span class="bg-red-100 text-red-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-red-200 dark:text-red-800 ml-3">hapus</span></a>
                </div>
								@endif
